namespace EasyOne.WebSite.Admin.Contents
{
    ...
    public partial class PhotoPreview : AdminPage
    {
        ...
        {
            StringBuilder builder = new StringBuilder();
            string[] strArray = this.contentDataTable.Rows[0][col.Trim()].ToString().Split(new string[] { "$$$" }, StringSplitOptions.RemoveEmptyEntries);
            string str = SiteConfig.SiteInfo.VirtualPath + VirtualPathUtility.AppendTrailingSlash(SiteConfig.SiteOption.UploadDir);
            string str2 = "";
            foreach (string str3 in strArray)
            {
                string[] strArray2 = str3.Split(new char[] { '|' });
                builder.Append(strArray2[0] + "：");
                str2 = DataSecurity.UrlEncode(strArray2[1]);
                if (str2.IndexOf("http://", StringComparison.OrdinalIgnoreCase) >= 0)
                {
                    builder.Append("<a href='" + str2 + "' target='_blank'>" + str2 + "</a>");
                }
                else
                {
                    builder.Append("<a href='" + str + str2 + "' target='_blank'>" + str2 + "</a>");
                }
                builder.Append("<br/>");
            }
        ...
        protected string GetImgPath(string col, string col2)
        {
            string str = DataSecurity.UrlEncode(this.contentDataTable.Rows[0][col].ToString());
            string str2 = this.contentDataTable.Rows[0][col2].ToString();
            string virtualPath = SiteConfig.SiteInfo.VirtualPath + VirtualPathUtility.AppendTrailingSlash(SiteConfig.SiteOption.UploadDir);
            if (string.IsNullOrEmpty(str))
            {
                return ("<img src=\"" + virtualPath + "nopic.gif\" width=\"150\"> <br /> 还没有指定缩略图片");
            }
            string str4 = Utility.ConvertAbsolutePath(virtualPath, str);
            return ("<img src=\"" + str4 + "\" width=\"150\"> <br /> " + str2);
        }
        ...
        {
            if (string.IsNullOrEmpty(this.contentDataTable.Rows[0][col].ToString()))
            {
                return (base.Request.PhysicalApplicationPath + "//UploadFiles//nopic.gif");
            }
            return (base.Request.PhysicalApplicationPath + this.contentDataTable.Rows[0][col].ToString().Trim());
        }
